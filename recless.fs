...
*)
  ...
  member this.find_first() =
    for nonterminal in this.Nonterminals do //initialization
      this.First.Add(nonterminal, HashSet<string>())
    let mutable progress = true
    while progress do
      progress <- false
      for rule in this.Productions do  
        let mutable is_nullable = true
        let mutable i = 0 
        while is_nullable && i < rule.rhs.Count do  
          let symbol = rule.rhs.[i]
          if not(this.Nonterminals.Contains(symbol)) && this.Symbols.Contains(symbol) then //symbol is terminal
            progress <- this.First.[rule.lhs].Add(symbol) || progress
            is_nullable <- false
          else //symbol is a nonterminal, append its first set to lhs' first set 
            let count = this.First.[rule.lhs].Count    
            this.First.[rule.lhs].UnionWith(this.First.[symbol])
            progress <- count <> this.First.[rule.lhs].Count || progress
            if not(this.Nullable.Contains(symbol)) then //symbol is not nullable
              is_nullable <- false
            //check if nullable
  ...
  member this.firstseq (seq:Vec<string>, starti) =
    ...
    while i < seq.Count && not(stop) do
      let symbol = seq.[i]
      //check if symbol is a terminal
      if not(this.Nonterminals.Contains(symbol)) && this.Symbols.Contains(symbol) then
        setax.Add(symbol) |> ignore
        stop <- true
      else
        setax.UnionWith(this.First.[symbol])
        //check if symbol is not nullable
        if not(this.Nullable.Contains(symbol)) then
            stop <- true
      i <- i + 1
    //while
    setax
  ...
  member this.find_follow() = 
    for nonterminal in this.Nonterminals do //initialization
      this.Follow.Add(nonterminal, HashSet<string>())
    this.Follow.["METASTART"].Add("EOF") |> ignore
    let mutable progress = true
    while progress do
      progress <- false
      for rule in this.Productions do
        ...
        while i < count do
          //if its the last symbol
          let symbol = rule.rhs.[i]
          if this.Nonterminals.Contains(symbol) then
            let num_follow = this.Follow.[symbol].Count
            //initialize firstseq
            let firstseq = HashSet<string>()
            if i <> count - 1 then
              //firstseq.UnionWith(this.firstseq(rule.rhs, i+1))
              this.Follow.[symbol].UnionWith(this.firstseq(rule.rhs, i+1))
            //if there is no following rule or the following rule(s) are nullable
            if i = count-1 || this.nullableseq(rule.rhs, i+1) then
              this.Follow.[symbol].UnionWith(this.Follow.[rule.lhs])
            //Add non null first sequence to follow set
            progress <- num_follow <> this.Follow.[symbol].Count || progress
          //if this
          i <- i + 1
        //while i
      //for rule
  ...
  member this.make_table() = 
    ...
    let mutable i = 0
    for rule in this.Productions do
      let first_set = this.firstseq(rule.rhs, 0)
      //for each terminal in the first set
      for terminal in first_set do
        if this.LL1Table.[rule.lhs].ContainsKey(terminal) then
          raise(Exception(sprintf "Table Entry contains multiple keys for nonterminal %s and terminal %s: rules %d and %d"
                            (rule.lhs) (terminal) (this.LL1Table.[rule.lhs].[terminal]) (i) ))
        this.LL1Table.[rule.lhs].Add(terminal, i)
      if this.nullableseq(rule.rhs, 0) then
        //for each terminal in the follow set of the rule
        for terminal in this.Follow.[rule.lhs] do
          //check if entry already exists
          for terminal in this.Follow.[rule.lhs] do
            if this.LL1Table.[rule.lhs].ContainsKey(terminal) && not(i = this.LL1Table.[rule.lhs].[terminal]) then
              raise(Exception(sprintf "Table Entry contains multiple keys for nonterminal %s and terminal %s: rules %d and %d"
                                (rule.lhs) (terminal) (this.LL1Table.[rule.lhs].[terminal]) (i) ))
          this.LL1Table.[rule.lhs].Add(terminal, i)
      i <- i + 1
